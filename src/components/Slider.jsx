...
const sliderData = [
  {
    ...
    textSub: "from the comfort of your home",
    image: "url",
  },
  {
  ...
  {
    text: "Get a Job, and flex your newly acquire skill",
...
function SliderBgImage({ slideCounter }) {
  if (slideCounter === 0) {
    return <img className="bgImage" src={SkillsImage} alt="Education" />;
  } else if (slideCounter === 1) {
    return <img className="bgImage" src={Design} alt="Education" />;
  } else if (slideCounter === 2) {
    return <img className="bgImage" src={JobImg} alt="Education" />;
  }
}
  ...
  return (
    <div className="hero">
      <div className="wecome-txt">
        <p className="top-text">{sliderData.text}</p>
        <p className="bottom-text">{sliderData.textSub}</p>
      </div>
      ...
      <div className="slider-index">
        <ul>
          <li
            onClick={() => setSlideCounter(0)}
            className={slideCounter === 0 ? "slider-dot green" : "slider-dot"}
          ></li>
          <li
            onClick={() => setSlideCounter(1)}
            className={slideCounter === 1 ? "slider-dot green" : "slider-dot"}
          ></li>
          <li
            onClick={() => setSlideCounter(2)}
            className={slideCounter === 2 ? "slider-dot green" : "slider-dot"}
          ></li>
        </ul>
      </div>
      {/*
...
export default function Slider() {
  ...
  return (
    <div>
      ...
      <InfoText
        sliderData={sliderData[slideCounter]}
